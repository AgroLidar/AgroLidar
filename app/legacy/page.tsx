      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-20">
        <section className="w-full text-center">
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <a href="/simulator" className="rounded-md bg-emerald-400 px-5 py-2.5 text-sm font-semibold text-black">Launch Simulator</a>
            <a href="/docs" className="rounded-md border border-white/20 px-5 py-2.5 text-sm font-semibold text-white">View Docs</a>
          </div>
        </section>
        ...
        <section className="space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {problemCards.map((card) => (
              <article key={card} className="rounded-xl border border-white/10 bg-white/5 p-5 text-sm text-white/90">{card}</article>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-4">
            ...
            <ul className="grid grid-cols-2 gap-3 text-sm text-white/85 sm:grid-cols-3">
              {detectionTargets.map((target) => (
                <li key={target} className="rounded-md border border-white/10 bg-white/5 px-3 py-2 text-center capitalize">{target}</li>
              ))}
            </ul>
          </div>
        ...
        <section id="simulator" className="scroll-mt-24 rounded-2xl border border-white/10 bg-white/5 p-6">
          <SectionTitle title="Web Simulator" subtitle="Enter the playable browser-native AgroLidar simulator." />
          <div className="mt-4">
