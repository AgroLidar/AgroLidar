      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-20">
        <section className="w-full text-center">
          <div className="mx-auto flex max-w-3xl flex-wrap items-center justify-center gap-2">
            <span className="rounded-full border border-emerald-300/40 bg-emerald-300/10 px-3 py-1 text-xs font-medium text-emerald-200">
              Field-first autonomy
            </span>
            <span className="rounded-full border border-sky-300/40 bg-sky-300/10 px-3 py-1 text-xs font-medium text-sky-200">
              LiDAR safety layer
            </span>
        ...
        <section className="space-y-8">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {problemCards.map((card) => (
              <article
                key={card}
                className="rounded-xl border border-white/10 bg-white/5 p-5 text-sm text-white/90 backdrop-blur"
              >
                {card}
              </article>
            ))}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2 text-sm font-medium text-emerald-200">
              {pipeline.map((step, index) => (
                <span key={step} className="flex items-center gap-2">
                  <span className="rounded-md border border-emerald-300/30 bg-emerald-300/10 px-2.5 py-1">
                    {step}
                  </span>
        ...
        <section className="rounded-2xl border border-rose-300/20 bg-rose-300/5 p-6">
          ...
          <p className="mt-3 text-sm text-white/80">No online self-training in production</p>
        ...
        <section id="sandbox" className="rounded-2xl border border-white/10 bg-white/5 p-6">
          <h2 className="text-2xl font-semibold tracking-tight sm:text-3xl">Sandbox</h2>
          <p className="mt-2 text-sm text-white/75">
            Run the full local workflow and test the landing stack quickly.
          </p>
          <pre className="mt-4 overflow-x-auto rounded-xl border border-white/10 bg-black/40 p-4 text-xs text-emerald-200 sm:text-sm">
            <code>{`git clone https://github.com/AgroLidar/AgroLidar
make setup
make generate-data && make train && make serve`}</code>
          </pre>
        </section>
      </div>
    </main>
