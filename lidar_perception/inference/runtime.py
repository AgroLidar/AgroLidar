...
class InferenceRuntime:
    ...
    def __init__(self, config_path: str, checkpoint_path: str):
        ...
        self.device = torch.device(
            "cuda" if self.config.get("device") == "cuda" and torch.cuda.is_available() else "cpu"
        )
        ...
        self.default_vehicle_speed_mps = float(
            inference_config.get("default_vehicle_speed_mps", 3.0)
        )
        self.braking_deceleration_mps2 = float(
            inference_config.get("braking_deceleration_mps2", 2.5)
        )
    ...
    def _fuse_maps(
        self, occupancy: np.ndarray, distance_map: np.ndarray
    ) -> tuple[np.ndarray, np.ndarray]:
        self._occupancy_history.append(occupancy.copy())
        self._distance_history.append(distance_map.copy())
        weights = np.array(
            [
                self.occupancy_fusion_decay**idx
                for idx in range(len(self._occupancy_history) - 1, -1, -1)
            ],
            dtype=np.float32,
        )
        ...
        fused_distance = np.where(
            weight_sum > 0.0, fused_distance / np.maximum(weight_sum, 1e-6), 0.0
        )
    ...
    def _annotate_stop_zone(self, result: dict, vehicle_speed_mps: float) -> dict:
        ...
        for detection in result["detections"]:
            ...
            ttc_s = (
                detection["distance_m"] / max(closing_speed, 1e-6)
        ...
        result["min_time_to_collision_s"] = (
            float(min_ttc_s) if np.isfinite(min_ttc_s) else float("inf")
        )
    ...
    def infer_points(self, points, vehicle_speed_mps: float | None = None):
        ...
        fused_occupancy, fused_distance = self._fuse_maps(
            result["occupancy"], result["distance_map"]
        )
        ...
        result["nearest_obstacle_distance_m"] = (
            float(fused_distance[occupied].min()) if occupied.any() else float("inf")
        )
        ...
        speed = (
            self.default_vehicle_speed_mps
            if vehicle_speed_mps is None
            else float(vehicle_speed_mps)
        )
    ...
    def infer_file(self, point_cloud_path: str, vehicle_speed_mps: float | None = None):
        ...
        return self.infer_points(
            load_point_cloud(point_cloud_path), vehicle_speed_mps=vehicle_speed_mps
        )
