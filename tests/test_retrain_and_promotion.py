...
def test_retrain_writes_metadata_and_candidate_dir(tmp_path: Path, monkeypatch):
    cfg = {
        ...
        "model": {"name": "pointpillars_bev", "in_channels": 6, "base_channels": 8, "num_classes": 5, "num_segmentation_classes": 6, "score_threshold": 0.2, "nms_iou_threshold": 0.2, "max_detections": 8, "max_candidates_per_class": 8},
        "training": {"epochs": 1, "learning_rate": 0.001, "weight_decay": 0.0, "mixed_precision": False, "losses": {"detection": 1.0, "segmentation": 1.0, "obstacle": 0.5}},
        "evaluation": {"iou_threshold": 0.25, "dangerous_iou_threshold": 0.2, "latency_warmup": 1, "latency_iters": 1},
        "retrain": {"hard_case_ratio": 0.3, "oversample_dangerous_classes": True, "dangerous_class_weight": 2.0, "candidate_tag": "it"},
    }
    cfg_path = tmp_path / "retrain.yaml"
    cfg_path.write_text(yaml.safe_dump(cfg), encoding="utf-8")

    monkeypatch.setattr(retrain_script, "parse_args", lambda: type("Args", (), {"config": str(cfg_path), "resume": None})())
...
def test_promote_model_archives_previous_and_promotes_candidate(tmp_path: Path, monkeypatch):
    ...
    registry_path.write_text(
        json.dumps(
            [
                {"version": "v1", "timestamp": "2026-01-01T00:00:00+00:00", "status": "production", "checkpoint": "prod.pt", "config_hash": "", "dataset_manifest": "", "metrics": {}, "notes": ""},
                {"version": "v2", "timestamp": "2026-01-02T00:00:00+00:00", "status": "candidate", "checkpoint": "cand.pt", "config_hash": "", "dataset_manifest": "", "metrics": {}, "notes": ""},
            ]
        ),
        encoding="utf-8",
    )
    report = tmp_path / "comparison.json"
    report.write_text(json.dumps({"promote": True, "decision_reason": "ok", "candidate": {}}), encoding="utf-8")
